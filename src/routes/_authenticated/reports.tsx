...
import { inr, monthLabel, num, type Workbook } from "@/lib/sheets-schema";
...
function ReportsPage() {
  ...
  // Heavy derivations run once per workbook change, not on every render
  // (background refetches re-render this page often with staleTime: 0).
  const data = useMemo(() => (wb ? buildReportData(wb) : null), [wb]);
  ...
  return (
    ...
    >
      {fallback ?? (data ? <ReportBody data={data} /> : null)}
    </AppShell>
  );
}
...
function buildReportData(wb: Workbook) {
  const s = summary(wb);
  ...
  const arrears = [...s.dues]
    ...
    .slice(0, 10);
  return { s, series, kinds, deposits, occupancyData, arrears };
}

/** recharts is ~350KB — load it after first paint so the page never
 *  blocks on chart parsing (the main cause of the freeze on open). */
function useRecharts(): typeof import("recharts") | null {
...
function CollectionsChart({ series }: { series: ReportData["series"] }) {
  const r = useRecharts();
  if (!r) return <Skeleton className="h-64 w-full" />;
  const { Bar, BarChart, CartesianGrid, Legend, ResponsiveContainer, Tooltip, XAxis, YAxis } = r;
  return (
...
function OccupancyChart({ occupancyData }: { occupancyData: ReportData["occupancyData"] }) {
  const r = useRecharts();
  if (!r) return <Skeleton className="h-56 w-full" />;
...
function ReportBody({ data }: { data: ReportData }) {
  const { s, series, kinds, deposits, occupancyData, arrears } = data;
...
function EntryHistoryCard() {
  ...
  // Sort + filter only when the audit log or the query changes — the old code
  // re-sorted (slow localeCompare) and re-scanned every row on each render,
  // which froze the page while typing or during background refetches.
  // Timestamps are ISO strings, so plain string comparison orders correctly
  // and is far cheaper than localeCompare on low-end phones.
  const rows = useMemo(
    () =>
      (wb?.audit ?? []).slice().sort((a, b) => {
